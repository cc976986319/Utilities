...
namespace Utilities.Data.Cron.Observers
{
    ...
    ///                        └───── 月(1 - 12)     【允许的通配符[, -* /]】
    /// </remarks>
    public class MonthObserver : Base.Observer
    {
        /// <summary>
        /// 实例化 月 观察者
        /// </summary>
        /// <param name="resolve">cron解析器</param>
        public MonthObserver(CronResolve resolve)
        {
        ...
        {
            string _cron = GetTargetCronSlice(TimeFormat.Month, cron);
            Tuple<bool, string, dynamic> value = PipelineController.MonthDisposePipeline(_cron);
            if (value.Item1)
