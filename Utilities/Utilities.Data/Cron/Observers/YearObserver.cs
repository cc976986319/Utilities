...
namespace Utilities.Data.Cron.Observers
{
    ...
    ///                                    └ 年  【允许的通配符[, -* /] 非必填】
    /// </remarks>
    public class YearObserver : Base.Observer
    {
        /// <summary>
        /// 实例化 年 观察者
        /// </summary>
        /// <param name="resolve">cron解析器</param>
        public YearObserver(CronResolve resolve)
        {
        ...
        {
            string _cron = GetTargetCronSlice(TimeFormat.Year, cron);
            Tuple<bool, string, dynamic> value = PipelineController.YearDisposePipeline(_cron);
            if (value.Item1)
